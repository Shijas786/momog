...
void main(){
  ...
  // fbm2 returns values roughly in [-1.0, 1.0]
  float base = fbm2(uv, uTime * 0.05);
  // Map noise strictly to [0.0, 1.0] and scale it down so we get empty spaces
  base = (base * 0.5 + 0.5) * 0.6;
  
  // Apply density offset but keep it sparse
  float feed = base + (uDensity - 0.5) * 0.5;
...
function initPixelBlast(container, opts = {}) {
  const config = {
    variant: 'circle',
    pixelSize: 6,
    color: '#007d11',
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    const cleanup = initPixelBlast(containerRef.current, {
      variant,
      pixelSize,
      color,
      antialias,
  ...
  return <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0 }} {...props} />;
}
